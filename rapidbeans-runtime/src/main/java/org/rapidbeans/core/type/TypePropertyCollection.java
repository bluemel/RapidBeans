...
public class TypePropertyCollection extends TypeProperty {
	 ...
	 * proerties parent bean. Removing the link will also remove the inverse link.
	 */
	...
	/**
	 * @param defaultCharSeparator the defaultCharSeparator to set
	 */
	...
	/**
	 * @param defaultCharEscape the defaultCharEscape to set
	 */
	...
	/**
	 * @param charEscape the charEscape to set
	 */
	...
	/**
	 * RapidBean default Java collection implementation. It has a good performance
	 * also for big collections (LinkeHashSet doesn't). Elements are sorted as
	 * created and as serialized / persisted (XML not SQL). Please note that
	 * ArrayList does not prevent linking the same instance twice which complies to
	 * a sequence (ordered bag).
	 */
	public static final Class<?> DEFAULT_COLLECTION_CLASS_DEFAULT = ArrayList.class;

	/**
	 * defines the default collection class for all collections (= association roles
	 * or ends).
	 */
	private static Class<?> defaultCollectionClass = DEFAULT_COLLECTION_CLASS_DEFAULT;

	/**
	 * The collection class used for implementing this association role or end. The
	 * default is a LinkedHashSet which - prevents having the same object associated
	 * twice - preserves the order
	 */
	 ...
	 * 
	 * @param xmlNode        the XML DOM node with the property type description.
	 ...
	 * Constructor for TypePropertyCollection.
	 * 
	 * @param xmlNode              the XML DOM node with the property type
	 *                             description.
	 * @param parentBeanType       the parent bean type
	 * @param specificTypeNamePart "Collection"
	 * @param separator            sep
	 * @param escape
	 */
	public TypePropertyCollection(final XmlNode[] xmlNodes, final TypeRapidBean parentBeanType,
		...
		if (collectionClassname != null && !collectionClassname.equals("")) {
			try {
				...
				if (!ClassHelper.classOf(Collection.class, this.collectionClass)) {
					throwModelValidationException(
							"Invalid collectionclass: Class \"" + collectionClassname + " is not a Collection.");
				}
	...
	/**
	 * @return the name of the property of the target type that defines the inverse
	 *         link for this association.
	 */
	 ...
	 * collection class for all for all collections (= association roles or ends).
	 * 
	 * @param defColClass the default collection class
	 */
	 ...
	 * 
	 * @param clazz the new default collection class
	 */
	 ...
	 * 
	 * @param beantype the parent bean type
	 * @param propNode the XML property description node
	 */
