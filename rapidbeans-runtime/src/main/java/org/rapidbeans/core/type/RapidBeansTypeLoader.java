...
public final class RapidBeansTypeLoader {
	 ...
	 * @param elementName the XML element name
	 * @param typeName    the RapidBean type name
	 * @param override    if overriding the bind is allowed or not Handle overriding
	 *                    with care
	 */
	...
	/**
	 * removes a Rapid Beans type registration. Caution: for test purposes only!!!
	 * 
	 * @param typename the type's name
	 */
	public void unregisterType(final String typename) {
		if (this.typeMap.get(typename) == null) {
			throw new TypeNotFoundException("bean type \"" + typename + "\" not registered");
		}
	 ...
	 * </p>
	 * 
	 * @param typeclass the type's class { TypeRapidEnum, TypeRapidQuantity,
	 *                  TypeRapidBean }
	...
	/**
	 * CAUTION: this methods clears the single typeMap. Just use this for unit tests
	...
	/**
	 * CAUTION: this methods clears all generic bean types out of the single type
	 * map. Just use this for unit tests or if you exactly know what you're doing.
	 */
