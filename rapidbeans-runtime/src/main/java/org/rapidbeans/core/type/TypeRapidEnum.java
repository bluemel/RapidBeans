...
public final class TypeRapidEnum extends RapidBeansType {
	 ...
	 * 
	 * @throws org.rapidbeans.core.exception.EnumException if an element with the
		...
		if (e == null) {
			if (!this.getName().equals("org.rapidbeans.core.basic.RapidEnum")) {
				throw new EnumException(
						"element \"" + enumElementName + "\" is not part of enum \"" + this.getName() + "\"");
			}
		}
		return e;
	}
	 ...
	 * 
	 * @param otherType the other RapidEnum type to compare with
	 ...
	 * constructor for a type instance of a generic enum.
	 * 
	 * @param typenameOrDescr the fully qualified enum type name or the whole
	 *                        description
	 */
	...
	private void checkElements() {
		if (this.elements.size() == 0) {
			if (!this.getName().equals("org.rapidbeans.core.basic.RapidEnum")) {
				throw new EnumException(
						"Problems with enumeration type \"" + getName() + "\". An enum must at least have one member");
			}
		}
		int j;
	 ...
	 * 
	 * @return null if the enum with the specified type name could not be found and
	 *         a concrete enum class could not be loaded
	 */
