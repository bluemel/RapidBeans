...
public final class RapidBeanSerializer {
	 ...
	 * 
	 * @param rootBean the bean or root bean of the composite
	 */
				...
				if (!(file.exists())) {
					if (!file.createNewFile()) {
						throw new RapidBeansRuntimeException(
								"Could not create file \"" + file.getAbsolutePath() + "\"");
					}
				}
	 ...
	 * @param colProp the parent collection property
	 * @param bean    the bean
	 */
	private void write(final Writer osw, final int depth, final PropertyCollection colProp, final RapidBean bean) {
		try {
			...
			for (Property prop : bean.getPropertyList()) {
				...
				if (prop.getValue() != null && (!(proptype instanceof TypePropertyCollection)
