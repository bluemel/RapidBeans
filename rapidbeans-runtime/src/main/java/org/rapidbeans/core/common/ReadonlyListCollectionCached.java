...
public class ReadonlyListCollectionCached<T> extends ReadonlyListCollection<T> implements PropertyChangeListener {
	 ...
	 * this is not a problem. All users of to array must be aware that they will get
	 * a snapshot of the collection's current state.
	 * 
	...
	/**
	 * returns the collection converted to an array list. Since an arry list is not
	 * immutable this method is private.
	 * 
	 * @return the collection converted to an array
	 */
	 ...
	 * Collections that are no Lists will be converted to arrays which will cause
	 * performance problems in case a big collections.
	 * 
