...
public final class ManifestReader {
	...
	/**
	 * Read a manifest from the same jar file or class folder hierarchy of the given
	 * class.
	 * 
	 * @param clazz the class that lies in the jar file ore the same class folder
	 *              hierarchy.
	 * @return the manifest
	 */
	public static Manifest readManifestFromJarOfClass(final Class<?> clazz) {
		...
		final String pathToManifest = pathToThisClass.toString().substring(0,
				pathToThisClass.length() + 2 - ("/" + classFilePath).length()) + "/META-INF/MANIFEST.MF";
		Manifest manifest;
		try {
