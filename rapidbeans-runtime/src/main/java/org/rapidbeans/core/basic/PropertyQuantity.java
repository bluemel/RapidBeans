...
/**
 * A <b>RapidQuantity</b> bean property stores a quantity of a certain
 * type.<br/>
 * Attributes<br/>
 * <b>quantity: (mandatory)</b> specifies the bean RapidQuantity type
 * (class)<br/>
...
public class PropertyQuantity extends Property {
	...
	/**
	 * the encapsulated RapidQuantity value. !!! do not initialize here because the
	 ...
	 * 
	 * @param quantityValue the new value for this property.<br/>
		...
		if (argValue instanceof RapidQuantity) {
			...
			try {
				quantity = RapidQuantity.createInstance(
						((TypePropertyQuantity) this.getType()).getQuantitytype().getName(), (String) argValue);
	 ...
	 * 
	 * @param quantityValue the value to validate<br/>
	 *                      Must be an instance of the following classes:<br/>
	 *                      <b>RapidQuantity:</b> the quantity<br/>
	 *                      <b>String:</b> the quantity as String: &lt;number&gt;
	 *                      &lt;enum&gt;<br/>
	...
	public RapidQuantity validate(final Object quantityValue) {
		...
		if (quantity.getType() != quantPropType.getQuantitytype()) {
			...
			throw new ValidationException("invalid.prop.quantity.quantitytype", this,
					"Wrong type \"" + quantity.getType().getName() + "\" for new quantity \"" + quantity.toString()
							+ "\"\n" + "Expected quantity type \"" + quantPropType.getQuantitytype().getName() + "\"",
					sa);
		}
