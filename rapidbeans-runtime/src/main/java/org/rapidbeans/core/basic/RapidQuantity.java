...
public abstract class RapidQuantity implements Cloneable, Comparable<RapidQuantity> {
	 ...
	 * 
	 * @param typename determines the quantity's type
	 * @param val      determines the quantity's value e. g. "12 km"
	 * 
		...
		if (clazz != null) {
			try {
				...
				throw new RapidBeansRuntimeException(
						"RapidQuantity class \"" + clazz.getName() + "\" does not have a constructor with String", e);
			} catch (IllegalArgumentException e) {
				throw new RapidBeansRuntimeException(
						"IllegalArgumentException while trying" + "to create instance for quantity class \""
								+ clazz.getName() + "\", quantity type \"" + typename + "\"",
						e);
			} catch (InstantiationException e) {
				throw new RapidBeansRuntimeException(
						"InstantiationException while trying" + "to create instance for quantity class \""
								+ clazz.getName() + "\", quantity type \"" + typename + "\"",
						e);
			} catch (IllegalAccessException e) {
				throw new RapidBeansRuntimeException(
						"IllegalAccessException while trying" + "to create instance for quantity class \""
								+ clazz.getName() + "\", quantity type \"" + typename + "\"",
						e);
			} catch (InvocationTargetException e) {
				throw new RapidBeansRuntimeException(
						"InvocationTargetException while trying" + " to create instance for quantity class \""
								+ clazz.getName() + "\", quantity type \"" + typename + "\"",
						e);
			}
		} else {
	...
	public static final RapidQuantity createInstance(final TypeRapidQuantity type, final BigDecimal magnitude,
		...
		if (clazz != null) {
			try {
				...
				throw new RapidBeansRuntimeException("RapidQuantity class \"" + clazz.getName()
						+ "\" does not have a constructor with BigDecimal and RapidEnum", e);
			} catch (IllegalArgumentException e) {
				throw new RapidBeansRuntimeException(
						"IllegalArgumentException while trying" + "to create instance for quantity class \""
								+ clazz.getName() + "\", quantity type \"" + type.getName() + "\"",
						e);
			} catch (InstantiationException e) {
				throw new RapidBeansRuntimeException(
						"InstantiationException while trying" + "to create instance for quantity class \""
	 ...
	 * simple constructor.
	 * 
	 * @param description the enum's string representation.
	 */
	 ...
	 * constructor.
	 * 
	 * @param argType     the RapidQuantity's type
	 * @param description the enum's string representation.
	 */
	protected RapidQuantity(final TypeRapidQuantity type, final String description) {
		...
		default:
			...
			throw new ValidationException("invalid.quantity.format.threeormore", this, "Invalid quantity description \""
					+ description + "\" with more than two tokens.\n" + "Need two tokens: <magnitude> <unit>", oa3);
		}
	}
	 ...
	 * constructor.
	 * 
	 * @param argMagnitude the magnitude
	 * @param argUnit      the unit
	 */
	protected RapidQuantity(final BigDecimal argMagnitude, final RapidEnum argUnit) {
	 ...
	 * @param minimumFractionDigits the minimal fraction digits
	 * @param maximumFractionDigits the maximal fraction digits
	 * 
	 * @return the localized string
	 */
	public String toStringGui(final RapidBeansLocale locale, final int minimumFractionDigits,
		...
		return ft.format(this.getMagnitude()) + " " + this.getUnit().toStringGui(locale);
	}
	...
	public final boolean equals(final Object o) {
		...
		return (this.getUnit() == quant.getUnit()
				&& ((this.getMagnitude() == null && quant.getMagnitude() == null) || (this.getMagnitude() != null
	 ...
	 * 
	 * @param conversionUnit the unit to convert into
	 * 
	...
	public final RapidQuantity convert(final RapidEnum conversionUnit) {
		...
		if (this.getUnit() != conversionUnit) {
			final BigDecimal conversionFactor = this.getType().getConversionTable().getConversionFactor(this.unit,
					conversionUnit);
			if (conversionFactor == null) {
				throw new QuantityConversionNotSupportedException(
						"conversion of quantity class \"" + this.getClass().getName() + "\" from Unit \"" + this.unit
								+ "\" to Unit \"" + conversionUnit + "\" is not supported.");
			}
			...
			if (conversionFactorReciprocal) {
				try {
					...
					converted.magnitude = new BigDecimal(dMagnitude / dConversionFactor);
				}
			} else {
				converted.magnitude = (BigDecimal) this.magnitude.multiply(conversionFactor);
			}
	...
	/**
	 * Parser function for quantity units. To be overwritten for specific quantity
	 * classes.
	 * 
	 * @param token the token
	 * 
	 ...
	 * 
	 * @param fraction specifies the number of digits behind the decimal sign
	 */
	...
	/**
	 * Add the given summand to this. The given summand is converted to this unit.
	 * 
	...
	/**
	 * Divide this value by the given dividend. The given dividend is converted to
	 * this unit.
	 * 
	 ...
	 * Round the quantity.
	 * 
	 * @param friction     specifies the number of digits behind the decimal sign
	 * @param roundingMode rounding mode
	 */
	public RapidQuantity round(final int fraction, RoundingMode roundingMode) {
		if (this.magnitude == null) {
			throw new IllegalArgumentException("can not round a quantity with null magnitude");
		}
		int precision = 3;
