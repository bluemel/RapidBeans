...
public class PropertyString extends Property {
	 ...
	 * For a String property this means just verifying that the given object is a
	 * String.
	 * 
		...
		if ((!type.getEmptyValid()) || (!newStringValue.equals(""))) {
			...
			if ((maxLength != TypePropertyString.LENGTH_UNLIMITED) && (newStringValue.length() > maxLength)) {
				if (this.getBean() != null) {
					throw new ValidationException("invalid.prop.string.maxlen", this,
					...
					throw new ValidationException(
							"invalid.prop.string.maxlen", this, "Property \"" + this.getType().getPropName() + "\": "
									+ "value longer than max length " + maxLength,
							new String[] { newStringValue, Integer.toString(maxLength) });
				}
			}
			...
			if ((minLength > 0) && (newStringValue.length() < minLength)) {
				throw new ValidationException("invalid.prop.string.minlen", this,
						"new value shorter than" + " minimal length " + minLength,
			...
			if ((pattern != null) && (newStringValue != null) && !pattern.matcher(newStringValue).matches()) {
				...
				throw new ValidationException("invalid.prop.string.pattern", this,
						"value does not match pattern \"" + pattern.pattern() + "\".", oa);
			}
		}
		return newStringValue;
	}
