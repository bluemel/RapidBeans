...
public abstract class RapidBeanImplParent implements RapidBean, Link {
	...
	/**
	 * @param beanState the new state to set
	 */
	 ...
	 * 
	 * @return an array of parent beans starting with the (document) root and ending
	 *         with the direct parent
	 */
	 ...
	 * 
	 * @return if sorting can be accomplished according to the given property types
	 */
		...
		for (int i = 0; i < len; i++) {
			...
			default:
				throw new UtilException(
						"expandBeanPropertyValues(\"" + pattern + "\") failed\n" + "wrong state " + state);
			}
		}
		if (state == 1) {
	 ...
	 * ether a simple name or a chain of reference (collection, map) properties. The
	 * most right property can be an arbitrary one.
	 * 
