...
public final class TimeOfDay extends Time {
	...
	public BigDecimal parseMagnitudeOneToken(final String token) {
		...
		if (token.matches("[0-9]?[0-9]")) {
			...
			magnitude = new BigDecimal(
					Integer.parseInt(token.substring(0, 2)) * 60 + Integer.parseInt(token.substring(3, 5)));
		} else if (token.matches("[0-9]:[0-9][0-9]")) {
			magnitude = new BigDecimal(
					Integer.parseInt(token.substring(0, 1)) * 60 + Integer.parseInt(token.substring(2, 4)));
	...
	public UnitTime parseUnitOneToken(final String token) {
		...
		if (token.matches("[0-9]?[0-9]")) {
			...
			throw new ValidationException("invalid.prop.timeofday.parse.unit", this,
