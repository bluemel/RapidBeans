...
public final class IdMap {
	...
	public void insert(final RapidBean bean) {
		...
		if (this.findBean(typename, id) != null) {
			throw new DatasourceException(
					"Failed to insert bean \"" + typename + "::" + id + "\" into pool.\n" + "It is already in there.");
		}
		submap.put(id, bean);
	}
	...
	public void delete(final RapidBean bean) {
		...
		if (pooledBean == null) {
			throw new DatasourceException(
					"Failed to insert bean \"" + id + "\" out of pool.\n" + "No bean of this type is in there.");
		}
		map.remove(id);
		if (map.size() == 0) {
	 ...
	 * @param id       the bean's id as string
	 * 
	...
	/**
	 * implements a recursive traversal through the whole inheritance tree to fill
	 * the result set with all objects of this type and all subtypes.
	 * 
	 * @param type the type of beans to search for
	 * @param all  the collection to fill with all objects
	 */
