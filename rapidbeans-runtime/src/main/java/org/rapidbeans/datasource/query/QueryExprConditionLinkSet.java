...
final class QueryExprConditionLinkSet extends QueryExpression {
	 ...
	 * constructor.
	 * 
	 * @param colPropName the Collection Property's name
	 * @param mult        theCollection Property's (min? max?) multiplicity
	 ...
	 * 
	 * @param db          the data container
	 * @param resultSetIn the collection with found beans
	 * 
	...
	public List<RapidBean> eval(final Container db, final List<RapidBean> resultSetIn) {
		...
		for (RapidBean curBean : resultSetIn) {
			...
			if (this.linkname.equals("parentBean")) {
				...
				if ((curLinkSet != null) && (((this.multiplicity == -1) && (curLinkSet.size() > 0))
