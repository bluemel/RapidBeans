...
public class Document extends ContainerImpl {
	...
	/**
	 * constructs a Document by reading the XML file. The specified file's URL will
	 * be new the document's name.
	 * 
	 * @param rbType  the root bean type
	...
	public Document(final String docname, final TypeRapidBean rbType, final File docfile) {
		try {
			...
			throw new RapidBeansRuntimeException(
					"Error while deserializing document from file \"" + docfile.getAbsolutePath() + "\"", e);
		}
	}
	 ...
	 * constructs a Document by reading the XML file specified by the given URL.
	 * 
	 * @param rbType  the RapidBeanType of the new document's root bean
	 * @param docfile the XML file
	 */
	...
	public Document(final String docname, final TypeRapidBean rbType, final URL docurl) {
		try {
			...
			throw new RapidBeansRuntimeException(
					"Error while deserializing document from file \"" + docurl.toString() + "\"", e);
		}
	}
	...
	/**
	 * constructs a Document out of an existing bean which will be the document's
	 * root bean.
	 * 
	 * @param rootBean the document's root
	 */
	 ...
	 * write a bean's data to the persistent store.
	 * 
	 * @param useEncoding   prescribe an encoding to use for writing not matter what
	 *                      encoding the document had until now. If you leave this
	 *                      value empty the original encoding is preserved.
	 * 
	 * @param forceEncoding <br/>
	 *                      <li>true: the given encoding will be preferred if not
	 *                      null</li>
	 *                      <li>false: the original encoding will be preferred if
	 *                      not null</li>
	 * @param useUrl        the URL to use for writing. If it is null the file's URL
	 ...
	 * 
	 * @param bean       the bean to insert
	 * @param implicitly special for documents. Usually you do not explicitly insert
	 *                   beans into a document. Instead insert them implicitly by
	 *                   adding the to a parent bean. If you anyway try to insert
	 *                   explicitly the document tries to find an appropriate
	 *                   location according to the following strategy.
	 *                   <li>Find all composition collection properties that have
	 *                   the type of the bean to insert as target type.</li>
	 *                   <li>If there is exactly one add the bean there. Otherwise
	...
	public void insert(final RapidBean bean, final boolean implicitly) {
		if (this.idmap.findBean(bean.getType().getName(), bean.getIdString()) != null) {
			throw new BeanDuplicateException("messagedialog.create.duplicate", bean,
					"Bean \"" + bean.toString() + "\" already exists in document \"" + this.getName() + "\"",
	...
	/**
	 * find a single bean by query. Convenience method that also parses the query.
	 * 
	...
	public String getPath(final RapidBean bean, final char separator) {
		...
		while (o != null) {
			if (o instanceof RapidBean) {
				...
				throw new RapidBeansRuntimeException(
						"Unexpected parent class \"" + o.getClass().getName() + "\"for bean tree model");
			}
			...
			for (Property prop : bean.getPropertyList()) {
				if (prop instanceof PropertyCollection) {
					...
					if (colPropType.getInverse() != null) {
						inverseColPropType = (TypePropertyCollection) colPropType.getTargetType()
	 ...
	 * @param visitor carries the bean processing template method
	 * @param bean    the current tree node
	 */
	@SuppressWarnings("unchecked")
