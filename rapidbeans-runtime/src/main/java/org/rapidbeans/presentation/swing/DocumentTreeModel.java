...
public final class DocumentTreeModel implements TreeModel {
	 ...
	 * setter.
	 * 
	 * @param show determines if bean links are presented to the tree view.
	 */
	 ...
	 * setter.
	 * 
	 * @param show determines if properties are presented to the tree view.
	 */
	 ...
	 * parent's child array. <code>parent</code> must be a node previously obtained
	 * from this data source. This should not return <code>null</code> if
	 * <code>index</code> is a valid index for <code>parent</code> (that is
	 * <code>index >= 0 && index < getChildCount(parent</code>)). The parent can be
	 ...
	 * 
	 * @param index  the index of the child tree node
	 * 
				...
				if (this.showBeanLinks) {
					...
					if (this.beanFilter == null) {
						...
						for (final PropertyCollection colProp1 : colList) {
							if (this.beanFilter
									.applies(((TypePropertyCollection) colProp1.getType()).getTargetType())) {
								if (i < index) {
									i++;
								} else {
									colProp = colProp1;
			...
			if (this.beanFilter == null) {
				...
				if (parent instanceof DocumentTreeNodePropColLink) {
					...
					throw new RapidBeansRuntimeException(
							"Unexpected parent class \"" + parent.getClass().getName() + "\" for bean tree model");
				}
			} else {
				int i = 0;
	 ...
	 * 
	 * @param parent the bean or PropertyCollection tree node. Must be a node
	...
	public int getChildCount(final Object parent) {
		...
		if (parent instanceof RapidBean) {
			...
			throw new RapidBeansRuntimeException(
					"Unexpected child class \"" + parent.getClass().getName() + "\"for bean tree model");
		}
		return count;
	}
	 ...
	 * <code>parent</code> or <code>child</code> don't belong to this tree model,
	 * returns -1.
	 * 
	 * @param parent a node in the tree, obtained from this data source
	 * 
	 ...
	 *         <code>child</code> or <code>parent</code> are <code>null</code> or
	 *         don't belong to this tree model
	 */
	@SuppressWarnings("unchecked")
		...
		if (parent instanceof RapidBean) {
			...
			if (this.showProperties) {
				...
				if (child instanceof RapidBean) {
					...
					throw new RapidBeansRuntimeException(
							"Unexpected child class \"" + child.getClass().getName() + "\" for bean tree model");
				}
				boolean found = false;
				for (PropertyCollection colProp : colPropList) {
	...
	public void valueForPathChanged(final TreePath path, final Object newValue) {
		TreeModelEvent e = new TreeModelEvent(this, path,
				new int[] { this.getIndexOfChild(path.getParentPath().getLastPathComponent(), newValue) },
				new Object[] { newValue });
	 ...
	 * Properties.
	 * 
	 * @param argObject         the object: ether a bean or a
	 *                          BBPropCollctionTreeNode
	 * @param includeObjectSelf if the object given should also be included in the
	 *                          path
	 * 
	 ...
	 * 
	 * @param root the root bean or property with changes below
			...
			if (treePath == null) {
				...
				if (treePath.getLastPathComponent() instanceof DocumentTreeNodePropCol
						&& ((DocumentTreeNodePropCol) presentedObject)
								.getColProp() == ((DocumentTreeNodePropCol) treePath.getLastPathComponent())
										.getColProp()) {
					break;
				}
			} else {
	 ...
	 * 
	 * @param path         the parent path for these changes
	 * @param childIndices the child indices
	 * @param beans        the child objects
	 */
