...
public class MenuHistoryOpenDocumentSwing extends MenuHistoryOpenDocument {
	...
	/**
	 * Helper flag to eliminate two consecutive separators while the history menu is
	 * switched off.
	 */
			...
			if (entry instanceof MenuHistoryOpenDocumentSwing) {
				if (entry == this) {
					...
					if (this.getPresentation() == MenuHistoryOpenDocumentPresentationMode.inline
						...
						while ((nextEntry == null && indexItems < itemCount) || ((!this.separatorRemoved)
	 ...
	 * 
	 * @param selectedMenuItem the menu item that has been selected
	 */
