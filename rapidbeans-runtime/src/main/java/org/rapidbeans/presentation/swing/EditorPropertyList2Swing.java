...
public class EditorPropertyList2Swing extends EditorPropertySwing implements EditorBeanListener {
	 ...
	 * @param ed                    the parent editor
	 * @param provideOnlyValidInOut switch to provide only valid values in the "out"
	 *                              list
	 */
	public EditorPropertyList2Swing(final Application client, final EditorBean bizBeanEditor, final Property prop,
		...
		if (prop instanceof PropertyChoice) {
			...
			this.listIn.setCellRenderer(
					new RendererListCollection(bizBeanEditor.getDocumentView().getDocument(), this.getLocale()));
		} else {
		...
		try {
			...
			if (this.getProperty() instanceof PropertyCollection) {
				...
				throw new RapidBeansRuntimeException(
						"Unexpected property class \"" + this.getProperty().getClass().getName() + "\".");
			}
			try {
			...
			if (!addFailed) {
				final PropertyChangeEvent[] proparray = { new PropertyChangeEvent(this.getProperty(), oldValue,
						this.getProperty().getValue(), PropertyChangeEventType.set, null) };
	 ...
	 * 
	 * @param e the validation exception to indicate
	 */
	...
	public void updateUI() {
		if (this.listIn.getModel() instanceof ModelListCollection) {
			...
			throw new RapidBeansRuntimeException(
					"Unknown list model class \"" + this.listIn.getModel().getClass().getName() + "\"");
		}
		this.listIn.repaint();
		...
		if (this.listOut.getModel() instanceof ModelListCollectionAllWithout) {
			...
			throw new RapidBeansRuntimeException(
					"Unknown list model class \"" + this.listOut.getModel().getClass().getName() + "\"");
		}
		this.listOut.repaint();
	}

	/**
	 * Dummy implementation (not used). Data binding with collections should to the
	 * job.
	 * 
