...
public final class DocumentTreeCellRenderer extends DefaultTreeCellRenderer {
	...
	/**
	 * Sets the value of the current tree cell to value. If selected is true, the
	 * cell will be drawn as if selected. If expanded is true the node is currently
	 * expanded and if leaf is true the node represets a leaf and if hasFocus is
	 * true the node currently has focus. tree is the JTree the receiver is being
	 * configured for. Returns the Component that the renderer uses to draw the
	 * value.
	 * 
	 * @param tree        the tree
	 * @param value       the bean or Collection Property to render
	 * @param isSelected  if the tree node is selected
	 * @param expanded    if the tree node is expanded
	 * @param leaf        if the tree node is a leaf
	 * @param row         the row
	 * @param hasGotFocus if the tree node has the focus
	 * 
	 ...
	 * 
	 * @param bean the bean for which we need a text
	 * 
	...
	private String findText(final RapidBean bean) {
		...
		if (text == null) {
			try {
				if (bean.getContainer() != null && bean.getContainer() instanceof Document) {
					final List<String> sa = StringHelper
							.split("treeview.beanlabel." + ((Document) bean.getContainer()).getPath(bean, '.'), ".");
					if (sa.size() > 3) {
	 ...
	 * 
	 * @param colNode the bean for which we need a text
	 * 
	 * @return the found text or null if none found
	 */
	 ...
	 * 
	 * @param bean the bean for which we need a text
	 * @param link determines if it is a bean or a link to a bean.
	 * 
	 * @return the found text or null if none found
	 */
	private static ImageIcon findIcon(final RapidBean bean, final boolean link) {
		...
		if (ApplicationManager.getApplication() != null && ApplicationManager.getApplication().getMainwindow() != null
			...
			icon = ((MainWindowSwing) ApplicationManager.getApplication().getMainwindow()).getIconManager()
					.getIcon(bean.getType());
		}
	 ...
	 * 
	 * @param colNode the collection property node for which we query an icon
	 * 
	...
	private static ImageIcon getIconBeanRoot() {
		if (iconBeanRoot == null) {
			final String relPath = "pictures/root.gif";
			final URL urlPicRoot = new Application().getClass().getResource(relPath);
			if (urlPicRoot == null) {
				throw new RapidBeansRuntimeException(String.format("Could not load resource \"%s.class//%s\"",
						Application.class.getName(), relPath));
			}
			iconBeanRoot = new ImageIcon(urlPicRoot);
		}
		return iconBeanRoot;
	}
	...
	private static ImageIcon getIconBeanLink() {
		if (iconBeanLink == null) {
			iconBeanLink = new ImageIcon(new Application().getClass().getResource("pictures/beanlink.gif"));
		}
		return iconBeanLink;
	}
	...
	private static ImageIcon getIconBean() {
		if (iconBean == null) {
			iconBean = new ImageIcon(new Application().getClass().getResource("pictures/bean.gif"));
		}
		return iconBean;
	}
	...
	private static ImageIcon getIconColPropComposition() {
		if (iconColPropComposition == null) {
			iconColPropComposition = new ImageIcon(new Application().getClass().getResource("pictures/property.gif"));
		}
	...
	private static ImageIcon getIconColPropLink() {
		if (iconColPropLink == null) {
			iconColPropLink = new ImageIcon(new Application().getClass().getResource("pictures/propertyLink.gif"));
		}
