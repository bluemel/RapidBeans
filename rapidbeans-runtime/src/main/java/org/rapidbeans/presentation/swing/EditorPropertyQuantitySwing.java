...
public class EditorPropertyQuantitySwing extends EditorPropertySwing {
		...
		try {
			...
			if (value == null) {
				...
				if (value.getUnit() == null) {
					this.combobox
							.setSelectedItem(((TypePropertyQuantity) (this.getProperty().getType())).getDefaultUnit());
				} else {
		...
		ParsedNumber(final String s, final Locale l) {
			if (s != null && s.length() > 0) {
				try {
					...
					throw new ValidationException("invalid.quantity.magnitude.only", s,
							"\"" + s + "\" is not a valid number.", new Object[] { s });
				}
			}
		}
	 ...
	 *         get after appending additional characters.
	 * 
	 * @param completenessRequired if the input fields must be completeS
	 */
