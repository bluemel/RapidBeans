...
public abstract class EditorProperty implements View, DocumentChangeListener, PropertyChangeListener {
	...
	/**
	 * @param bakprop the propertyBak to set
	 */
	 ...
	 * UI accoring to a bean's contents.
	 */
	 ...
	 * @param bakProp       the bean backup property used instead of a transaction
	 * 
	...
	public static EditorProperty createInstance(final Application client, final EditorBean bizBeanEditor,
		...
		if (classname == null) {
			if ((prop.getBean() instanceof User || TypeRapidBean
					.isSameOrSubtype(TypeRapidBean.forName("org.rapidbeans.security.User"), prop.getBean().getType()))
	 ...
	 * 
	 * @param prop       the bean property to edit
	 * @param bakProp    the backup property used instead of a transaction
	 * @param beanEditor the parent bean editor
	 * @param client     the client
	 */
