...
public abstract class EditorBean implements View, EditorPropertyListener, DocumentChangeListener {
	 ...
	 * 
	 * @param propEditor the property editor where an input field has been changed
	 */
	...
	/**
	 * flag that indicates if in new mode the bean is already added to the parent
	 * collection property.
	 */
	 ...
	 * @param newBeanParentColProp a new Bean's parent collection property. Is not
	 *                             null if a new Bean is to be created Is null if an
	 *                             existing bean is simply edited
	 */
				...
				for (final ConfigPropEditorBean cfgprop : this.configuration.getPropertycfgs()) {
					...
					if (prop == null) {
						...
						throw new RapidBeansRuntimeException(
								"Invalid property \"" + cfgprop.getName() + "\"" + " configured for class \""
	 ...
	 * @param bizBean       the bean to edit
	 * @param newBeanParent a new Bean's parent collection property. Is not null if
	 *                      a new Bean is to be created Is null if an existing bean
	 *                      is simply edited
	 * 
	...
	/**
	 * determines the way how the editor behaves after pressing the apply button.
	 */
	private CreateNewBeansEditorApplyBehaviour createApplyMode = CreateNewBeansEditorApplyBehaviour.resetall;

	/**
	 * Setter to determine the way how the editor behaves after pressing the apply
	 * button.
	 * 
	 * @param mode determines the way how the editor behaves after pressing the
	 *             apply button.<br/>
	 *             <code>CREATE_APPLY_RESET_ALL</code>: all property values are
	 *             reset and have to be specified in order to create the next
	 *             bean.<br/>
	 *             <code>CREATE_APPLY_RESET_KEY</code>: only key property values are
	 *             reset. All other property values stay the same and can be reused
	 *             for creating the next bean.<br/>
	...
	private void resetBean() {
		...
		default:
			throw new RapidBeansRuntimeException(
					"Invalid createApplyMode " + this.createApplyMode + " for bean editor.");
		}
	 ...
	 * @param linkBack   if the bean is completely linked (with forward and inverse
	 *                   linked) with another bean or not
	 * @param updateUI   determines if the UI will be updated or not e. g. for
	 *                   normalization
	 */
	protected void validateInputAndUpdateBean(final boolean showDialog, final boolean linkBack,
			final boolean updateUI) {
	 ...
	 * 
	 * @param showDialog        if dialogs should be shown
	 * @param linkBack          if the bean is completely linked (with forward and
	 *                          inverse linked) with another bean or not
	 * @param updateUI          determines if the UI will be updated or not e. g.
	 *                          for normalization
	 * @param propEditorChanged the changed property editor. This is null if an OK
	 ...
	 * 
	 * @param setAdded set bean added afterwards. Usually this is exactly what you
	 *                 want to do.
	 */
	 ...
	 * 
	 * @return a HashMap with button wigets. The keys are the button names ok, apply
	 *         and cancel
	 */
	public abstract HashMap<String, Object> getButtonWidgets();

	/**
	 * indicates if the editor is for creating a new bean instead of just modifying
	 * an existing one.
	 * 
