...
public class QueryTest extends TestCase {
	...
	/**
	 * Querying an association end attribute directly leads to a string comparison
	 * on the "frozen" string value.
	 */
	...
	public void testFindByQueryStringAttrAssocEndNoCompOtherProp() {
		final Query query = new Query(
				"org.rapidbeans.test.addressbook5.AddressbookInDb[" + "owner[accountname = 'user1']]");
		assertEquals(1, testdoc02.findBeansByQuery(query).size());
	}
	...
	/**
	 * test complex query that uses two conditions on different associated objects
	 */
	public void testFindByQueryStringAttrTwoAssocsAnd() {
		Document doc = createTestDocument(false);
		Collection<RapidBean> resultSet = doc.findBeansByQuery(
				"TrainingDate[parentBean[from = '20060101'] & defaulttrainer[lastname = 'Dahlheimer']]");
		assertEquals(2, resultSet.size());
	}
	...
	/**
	 * simple path query that uses the sorting of the underlying collection class In
	 * this case a simple ArrayList is used for property trainers. So we expect to
	 * get the objects in the sequence we created them.
	 */
	...
	public void testPathQuery3rdLevelComplexConditions() throws IOException {
		ConfigSubmenu sm = (ConfigSubmenu) testdoc01.findBeanByQuery("/mainwindow/menubar/menus"
