...
public class DocumentTreeViewSwingTest extends TestCase {
	...
	/**
	 * Select Trainer bean "Bl�mel" and "Dautovic" in the tree view and delete them.
	 * => - the beans should be deleted - the tree view should be correctly updated
	 * 
	...
	public void testDeleteBeansFromTree() throws InterruptedException {
		...
		try {
			...
			assertEquals("Dautovic",
					((RapidBean) tree.getPathForRow(4).getLastPathComponent()).getProperty("lastname").getValue());
			...
			assertEquals(3, ((Collection<?>) ((DocumentTreeNodePropColComp) tree.getPathForRow(6)
					.getPath()[tree.getPathForRow(6).getPath().length - 2]).getColProp().getValue()).size());
	...
	public void testCreateSngleBeanViaCreateAction() throws InterruptedException {
		...
		try {
			...
			assertEquals("Dautovic",
					((RapidBean) tree.getPathForRow(4).getLastPathComponent()).getProperty("lastname").getValue());
			assertEquals("Meyer",
					((RapidBean) tree.getPathForRow(5).getLastPathComponent()).getProperty("lastname").getValue());
			...
			assertEquals("Meyer",
					((RapidBean) tree.getPathForRow(5).getLastPathComponent()).getProperty("lastname").getValue());
			assertEquals("Mayer",
					((RapidBean) tree.getPathForRow(6).getLastPathComponent()).getProperty("lastname").getValue());
