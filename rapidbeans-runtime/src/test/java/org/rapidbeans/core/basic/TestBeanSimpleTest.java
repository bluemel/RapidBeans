...
public class TestBeanSimpleTest {
	...
	@Test(expected = ValidationException.class)
	public void testIntegerPropertyInvalidGenericAccess()
	...
	public void testChoicePropertySingleGenericAccess() {
		...
		Assert.assertEquals(Sex.female,
				((ReadonlyListCollection<RapidEnum>) bean.getProperty("sex").getValue()).get(0));
	}
	...
	public void testChoicePropertyMultiple() {
		...
		TestBeanSimple bean = new TestBeanSimple();
		assertListsEqual(Arrays.asList(
				new RapidEnum[] { tlang.elementOf("german"), tlang.elementOf("english"), tlang.elementOf("french"), }),
				bean.getLanguages());
		assertListsEqual(Arrays.asList(new RapidEnum[] { Lang.german, Lang.english, Lang.french }),
				bean.getLanguages());
		bean.setLanguages(Arrays.asList(new Lang[] { Lang.chinese, Lang.spanish }));
		assertListsEqual(Arrays.asList(new RapidEnum[] { Lang.chinese, Lang.spanish }), bean.getLanguages());
	}
	...
	public void testAssociationPropertyMultiple() {
		...
		father.setChildren(Arrays.asList(new TestBeanSimple[] { son1, son2, son3, son4 }));
	...
	public void testAssociationPropertyMultipleGenericAcces() {
		...
		father.setPropValue("children", Arrays.asList(new TestBeanSimple[] { son1, son2, son3, son4 }));
	...
	public void testAssociationPropertyMultipleInvalid() {
		TestBeanSimple bean = new TestBeanSimple();
		bean.setChildren(Arrays.asList(new TestBeanSimple[] { new TestBeanSimple(), new TestBeanSimple(),
				new TestBeanSimple(), new TestBeanSimple(), new TestBeanSimple() }));
	}

	@Test(expected = ValidationException.class)
	public void testAssociationPropertyMultipleInvalidGenericAcces() {
		TestBeanSimple bean = new TestBeanSimple();
		bean.setPropValue("children", Arrays.asList(
				new TestBean[] { new TestBean(), new TestBean(), new TestBean(), new TestBean(), new TestBean() }));
	}
	...
	private void assertListsEqual(List<?> list1, List<?> list2) {
		...
		for (int i = 0; i < len; i++) {
			if (!list1.get(i).equals(list2.get(i))) {
				Assert.fail("List elements [" + Integer.toString(i) + "] \"" + list1.get(i).toString() + "\" and \""
						+ list2.get(i).toString() + "\" differ");
			}
		}
	}
}
