...
public class GenericBeanTest {
	 ...
	 * declarative type without a Java class definition). If you use this method the
	 * bean's type is registered a the bean type loader.
	 */
	@Test
	...
	public void testChoicePropertyMultiple() {
		...
		bean.setPropValue("languages",
				(Arrays.asList(new RapidEnum[] { tlang.elementOf("chinese"), tlang.elementOf("spanish") })));
		assertListsEqual(Arrays.asList(new RapidEnum[] { tlang.elementOf("chinese"), tlang.elementOf("spanish") }),
				(List<RapidEnum>) bean.getPropValue("languages"));
	}
	...
	public void testOverriding() {
		GenericBean bean = (GenericBean) RapidBeanImplParent
	...
	public void testAssociationPropertyMultiple() {
		...
		father.setPropValue("children", Arrays.asList(new GenericBean[] { son1, son2, son3, son4 }));
