...
public class IdKeypropsTest extends TestCase {
	...
	public void testSortingArrayListNoSorting() {
		try {
			...
			unregisterType(new AddressBook().getType().getName());
			unregisterType(new Person().getType().getName());
		}
	}

	private void unregisterType(String fullyQualifiedTypeName) {
		try {
			RapidBeansTypeLoader.getInstance().unregisterType(fullyQualifiedTypeName);
