...
public class XmlHelperTest {
	...
	public void testRetrieveNodeTopLevel() {
		Node tlNode = XmlHelper.getDocumentTopLevel(new File("src/test/resources/xml/textXmlComplex01.xml"))
				.getParentNode();
	...
	public void testRetrieveSubnodeWithSpecNodeText() {
		Node tlNode = XmlHelper.getDocumentTopLevel(new File("src/test/resources/xml/web.xml")).getParentNode();
		Assert.assertEquals("param-name",
				XmlHelper.getNode(tlNode, "//web-app/context-param/param-name[.='config_home']").getNodeName());
		Assert.assertEquals("config_home", XmlHelper
				.getNode(tlNode, "//web-app/context-param/param-name[.='config_home']").getFirstChild().getNodeValue());
	}
	...
	public void testRetrieveSubnodeWithSpecAttribute() {
		...
		Assert.assertEquals("view",
				XmlHelper
						.getNode(tlNode, "//workbench/window/page/views/view[@id='org.eclipse.jdt.ui.PackageExplorer']")
						.getNodeName());
	}
	...
	public void testRetrieveSubnodeWithSpecAttributeComplex() {
		Node tlNode = XmlHelper.getDocumentTopLevel(new File("src/test/resources/xml/textXmlComplex01.xml"))
				.getParentNode();
		Assert.assertEquals("false", XmlHelper
				.getNode(tlNode, "//workbench/window/page/views" + "/view[@id='org.eclipse.jdt.ui.PackageExplorer']"
	...
	public void testRetrieveSubnodeWithSpecSubnode() {
		Document doc = XmlHelper.getDocument(new File("src/test/resources/xml/web.xml"));
		...
		Assert.assertEquals("param-name", XmlHelper
				.getNodes(doc, "//web-app/context-param/param-name[text()='config_home']").item(0).getNodeName());
		Assert.assertEquals("context-param",
				XmlHelper.getNodes(doc, "//web-app/context-param[param-name/.='config_home']").item(0).getNodeName());
		Assert.assertEquals("context-param", XmlHelper
				.getNodes(doc, "//web-app/context-param[param-name/text()='config_home']").item(0).getNodeName());
	}
