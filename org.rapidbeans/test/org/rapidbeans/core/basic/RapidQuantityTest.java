...
public final class RapidQuantityTest extends TestCase {
	...
	public void testCreateInstanceClass() {
		...
		assertEquals(2123456.789, len.convert(UnitLength.km)
				.getMagnitudeDouble());
	...
	public void testBBQuantity() {
		Length len = new Length(new BigDecimal("1234.567890123456789"),
				UnitLength.km);
	...
	public void testToString() {
		assertEquals("1234.567890123456789 km", new Length(new BigDecimal(
				"1234.567890123456789"), UnitLength.km).toString());
	}
	...
	public void testCompareTo() {
		...
		assertEquals(0,
				new Length("1000000000000000000 pm").compareTo(new Length(
						"1000 km")));
		assertEquals(1,
				new Length("1000000000000000001 pm").compareTo(new Length(
						"1000 km")));
	}
	...
	public void testConvert() {
		TypeRapidQuantityConversionTable convTable = new Length("1 m")
				.getType().getConversionTable();
		assertEquals(new BigDecimal("1E+3"),
				convTable.getConversionFactor(UnitLength.km, UnitLength.m));
		assertEquals(false, convTable.getConversionFactorReciprocalFlag(
				UnitLength.km, UnitLength.m));
		assertEquals(new BigDecimal("1E+3"),
				convTable.getConversionFactor(UnitLength.m, UnitLength.km));
		assertEquals(true, convTable.getConversionFactorReciprocalFlag(
				UnitLength.m, UnitLength.km));
		assertEquals(new BigDecimal("1E+2"),
				convTable.getConversionFactor(UnitLength.m, UnitLength.cm));
		assertEquals(false, convTable.getConversionFactorReciprocalFlag(
				UnitLength.m, UnitLength.cm));
		assertEquals(new BigDecimal("1E+2"),
				convTable.getConversionFactor(UnitLength.cm, UnitLength.m));
		assertEquals(true, convTable.getConversionFactorReciprocalFlag(
				UnitLength.cm, UnitLength.m));

		assertEquals("1001.123 m",
				new Length("100112.3 cm").convert(UnitLength.m).toString());
		assertEquals("1001.123 m",
				new Length("1001.123 m").convert(UnitLength.m).toString());
		assertEquals("100112.3 cm",
				new Length("1001.123 m").convert(UnitLength.cm).toString());

		convTable = new Time("1 h").getType().getConversionTable();
		assertEquals(new BigDecimal("60"),
				convTable.getConversionFactor(UnitTime.h, UnitTime.min));
		assertEquals(false, convTable.getConversionFactorReciprocalFlag(
				UnitTime.h, UnitTime.min));
		assertEquals(new BigDecimal("60"),
				convTable.getConversionFactor(UnitTime.min, UnitTime.h));
		assertEquals(true, convTable.getConversionFactorReciprocalFlag(
				UnitTime.min, UnitTime.h));
