...
public final class RapidBeanTest extends TestCase {
	...
	public void testCreateInstance() {
		...
		assertSame(
				TypeRapidBean
						.forName("org.rapidbeans.presentation.config.swing.ConfigApplicationSwing"),
				bean.getType());
		...
		PropertyChoice lnftype = (PropertyChoice) bean
	...
	public void testGetParentBeans() {
		...
		assertEquals("org.rapidbeans.presentation.config.ConfigApplication",
				parentBeans[0].getType().getName());
		assertEquals("Test", ((ConfigApplication) parentBeans[0]).getName());
		assertEquals("org.rapidbeans.presentation.config.ConfigMainWindow",
				parentBeans[1].getType().getName());
		assertEquals("mainwindow",
				((ConfigMainWindow) parentBeans[1]).getName());
		assertEquals("org.rapidbeans.presentation.config.ConfigMenubar",
				parentBeans[2].getType().getName());
		assertEquals("menubar", ((ConfigMenubar) parentBeans[2]).getName());
		assertEquals("org.rapidbeans.presentation.config.ConfigSubmenu",
				parentBeans[3].getType().getName());
		assertEquals("test", ((ConfigSubmenu) parentBeans[3]).getName());
		assertEquals("org.rapidbeans.presentation.config.ConfigSubmenu",
				parentBeans[4].getType().getName());
		assertEquals("test2", ((ConfigSubmenu) parentBeans[4]).getName());
		assertEquals("org.rapidbeans.presentation.config.ConfigSubmenu",
				parentBeans[5].getType().getName());
	...
	public void testGetParentCompColProperty() {
		...
		GenericBean cert = (GenericBean) testdoc.findBean("Certificate",
				"Fach�bungsleiter");
		assertNotNull(cert);
	...
	public void testToStringGuiTypeWithOwnType() {
		...
		assertEquals("BillingPeriod",
				billingPeriod.getType().toStringGui(locale, false, null));
		RapidBean bluemel = doc.findBean("Trainer", "Bl�mel_Martin");
	...
	public void testToStringGuiType() {
		...
		assertEquals(
				"Person",
				bluemel.getType().getSupertype()
						.toStringGui(locale, false, null));
	}
	...
	private static Document createTestDocument() {
		...
		GenericBean trainer2 = createTrainer("Dahlheimer", "Berit", false,
				certificatesMandatory);
		trainers.add(trainer2);
		GenericBean trainer3 = createTrainer("Dautovic", "Damir", false,
				certificatesMandatory);
