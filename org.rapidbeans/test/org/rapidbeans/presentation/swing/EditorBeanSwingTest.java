...
public class EditorBeanSwingTest extends TestCase {
	...
	/**
	 * verify that the button text changes if you type something invalid.
	 * 
	...
	// assertEquals("", ((JTextField)
	// editor.getPropEditors().get(0).getWidget()).getText());
	// assertEquals(null, editor.getBean().getProperty("firstname").getValue());
	// assertEquals("", ((JTextField)
	// editor.getPropEditors().get(1).getWidget()).getText());
	// assertEquals("OK", ((JButton) buttons.get("ok")).getText());
	// assertEquals(false, ((JButton) buttons.get("ok")).isEnabled());
	// assertEquals("Apply", ((JButton) buttons.get("apply")).getText());
	// assertEquals(false, ((JButton) buttons.get("apply")).isEnabled());
	...
	// ((JTextField)
	// editor.getPropEditors().get(0).getWidget()).setText("Bl�mel");
	// assertEquals("Bl�mel", ((JTextField)
	// editor.getPropEditors().get(0).getWidget()).getText());
	// editor.getPropEditors().get(0).fireInputFieldChanged();
	// assertEquals("Bl�mel",
	// editor.getBean().getProperty("lastname").getValue());
	// assertEquals(null, editor.getBean().getProperty("firstname").getValue());
	// assertEquals("", ((JTextField)
	// editor.getPropEditors().get(1).getWidget()).getText());
	// assertTrue(editor.isAnyInputFieldChanged());
	...
	//
	// ((JTextField)
	// editor.getPropEditors().get(1).getWidget()).setText("Martin");
	// editor.getPropEditors().get(1).fireInputFieldChanged();
	// // assert that something has changed and that the buttons
	// // a correcly enabled / disabled and have correct texts
	// // Since "Martin Bl�mel" already exists the apply button
	...
	// * @throws InterruptedException test
	// */
	// public void testCreateIsAnyInputFieldChangedChangeCheckBoxProperty()
	// throws InterruptedException {
	// DocumentTreeViewSwing docTreeView =
	// PresentationSwingTestHelper.createTestTreeView();
	// JTree tree = docTreeView.getTree();
	// try {
	// assertEquals("trainers", PresentationSwingTestHelper.getColPropName(tree,
	// 1));
	...
	// RapidBean newBean =
	// RapidBeanImplStrict.createInstance(((TypePropertyCollection)
	// prop.getType()).getTargetType().getName());
	// EditorBeanSwing editor = (EditorBeanSwing) docView.getEditor(newBean,
	// true);
	...
	// PropertyBoolean leaderProp = (PropertyBoolean)
	// leaderEditor.getProperty();
	...
	// if (propEditor.getProperty().getType().getPropName().
	// equals("certificates")) {
	// assertEquals(new ArrayList<RapidBean>(),
	// propEditor.getInputFieldValue());
	...
	// * @throws InterruptedException test
	// */
	// public void testEditIsAnyInputFieldChangedChangeCheckBoxProperty() throws
