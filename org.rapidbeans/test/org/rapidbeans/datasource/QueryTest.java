...
public class QueryTest extends TestCase {
    ...
    public void testFindByQueryStringTypeSimple() {
        ...
        assertEquals("Blümel_Martin", iter.next().getIdString());
        assertEquals("Blümel_Ulrike", iter.next().getIdString());
    ...
    public void testFindByQueryStringAttrSimpleId() {
        Document doc = createTestDocument(false);
        Collection<RapidBean> resultSet = doc.findBeansByQuery("Trainer[id = 'Blümel_Ulrike']");
        assertEquals(1, resultSet.size());
        Iterator<RapidBean> iter = resultSet.iterator();
        assertEquals("Blümel_Ulrike", iter.next().getIdString());
    }
    ...
    // doc.findBeansByQuery("Trainer['Blümel_Ulrike']");
    // assertEquals(1, resultSet.size());
    // Iterator<RapidBean> iter = resultSet.iterator();
    // assertEquals("Blümel_Ulrike", iter.next().getIdString());
    // }
     ...
     * as top level object - 3 Trainer objects: Blümel, Dahlheimer, Dautovic - 3
