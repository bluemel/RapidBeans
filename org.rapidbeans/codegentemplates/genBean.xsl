...
<xsl:if test="$codegen = 'joint'">
	...
	<xsl:text>// END manual code section</xsl:text><xsl:value-of select="$newline"/>
</xsl:if>
...
<xsl:text> * Rapid Bean class: </xsl:text><xsl:value-of select="$classname"/><xsl:text>.</xsl:text><xsl:value-of select="$newline"/>
<xsl:text> * </xsl:text>
...
<xsl:value-of select="$newline"/>
<xsl:value-of select="$indent1"/><xsl:text>/**</xsl:text><xsl:value-of select="$newline"/>
<xsl:value-of select="$indent1"/><xsl:text> * property references initialization.</xsl:text><xsl:value-of select="$newline"/>
<xsl:value-of select="$indent1"/><xsl:text> */</xsl:text><xsl:value-of select="$newline"/>
<xsl:value-of select="$indent1"/><xsl:text>public void initProperties() {</xsl:text><xsl:value-of select="$newline"/>

<xsl:if test="@extends">
	<xsl:value-of select="$indent2"/><xsl:text>super.initProperties();</xsl:text><xsl:value-of select="$newline"/>
</xsl:if>
<xsl:for-each select="property">
	<xsl:choose>
		<xsl:when test="@depends">
...
<xsl:for-each select="property">
	<xsl:if test="@changeeventhandlerpre = 'true'">
		...
		<xsl:value-of select="$newline"/>
		<xsl:value-of select="$indent1"/><xsl:text>/**</xsl:text><xsl:value-of select="$newline"/>
		<xsl:value-of select="$indent1"/><xsl:text> * pre change event handler for property "</xsl:text><xsl:value-of select="@name"/><xsl:text>".</xsl:text><xsl:value-of select="$newline"/>
		<xsl:value-of select="$indent1"/><xsl:text> *</xsl:text><xsl:value-of select="$newline"/>
		<xsl:value-of select="$indent1"/><xsl:text> * @param e the property change event.</xsl:text><xsl:value-of select="$newline"/>
		<xsl:value-of select="$indent1"/><xsl:text> */</xsl:text><xsl:value-of select="$newline"/>
		<xsl:value-of select="$indent1"/><xsl:text>private void </xsl:text>
		<xsl:value-of select="@name"/>
		<xsl:text>BeforeChange(final PropertyertyChangeEvent e) {</xsl:text><xsl:value-of select="$newline"/>
		<xsl:value-of select="$indent2"/><xsl:text>// BEGIN manual code section</xsl:text><xsl:value-of select="$newline"/>
		<xsl:value-of select="$indent2"/><xsl:text>// </xsl:text>
		<xsl:value-of select="$classname"/>
		<xsl:text>.</xsl:text>
		<xsl:value-of select="@name"/>
		<xsl:text>BeforeChange()</xsl:text><xsl:value-of select="$newline"/>
...
<xsl:for-each select="property">
	<xsl:if test="@changeeventhandlerpost = 'true'">
		...
		<xsl:value-of select="$newline"/>
		<xsl:value-of select="$indent1"/><xsl:text>/**</xsl:text><xsl:value-of select="$newline"/>
		<xsl:value-of select="$indent1"/><xsl:text> * post change event handler for property "</xsl:text><xsl:value-of select="@name"/><xsl:text>".</xsl:text><xsl:value-of select="$newline"/>
		<xsl:value-of select="$indent1"/><xsl:text> *</xsl:text><xsl:value-of select="$newline"/>
		<xsl:value-of select="$indent1"/><xsl:text> * @param e the property change event.</xsl:text><xsl:value-of select="$newline"/>
		<xsl:value-of select="$indent1"/><xsl:text> */</xsl:text><xsl:value-of select="$newline"/>
		<xsl:value-of select="$indent1"/><xsl:text>private void </xsl:text>
		<xsl:value-of select="@name"/>
		<xsl:text>Changed(final PropertyertyChangeEvent e) {</xsl:text><xsl:value-of select="$newline"/>
		<xsl:value-of select="$indent2"/><xsl:text>// BEGIN manual code section</xsl:text><xsl:value-of select="$newline"/>
		<xsl:value-of select="$indent2"/><xsl:text>// </xsl:text>
		<xsl:value-of select="$classname"/>
		<xsl:text>.</xsl:text>
		<xsl:value-of select="@name"/>
		<xsl:text>Changed()</xsl:text><xsl:value-of select="$newline"/>
		<xsl:value-of select="$indent2"/><xsl:text>// END manual code section</xsl:text><xsl:value-of select="$newline"/>
		<xsl:value-of select="$indent1"/><xsl:text>}</xsl:text><xsl:value-of select="$newline"/>
	</xsl:if>
</xsl:for-each>
...
<xsl:value-of select="$newline"/>
<xsl:value-of select="$indent1"/><xsl:text>/**</xsl:text><xsl:value-of select="$newline"/>
<xsl:value-of select="$indent1"/><xsl:text> * default constructor.</xsl:text><xsl:value-of select="$newline"/>
<xsl:value-of select="$indent1"/><xsl:text> */</xsl:text><xsl:value-of select="$newline"/>
...
<xsl:value-of select="$indent1"/><xsl:text> * @param s</xsl:text><xsl:value-of select="$newline"/>
<xsl:value-of select="$indent1"/><xsl:text> *            the string</xsl:text><xsl:value-of select="$newline"/>
<xsl:value-of select="$indent1"/><xsl:text> */</xsl:text><xsl:value-of select="$newline"/>
...
<xsl:value-of select="$newline"/>
<xsl:value-of select="$indent1"/><xsl:text>/**</xsl:text><xsl:value-of select="$newline"/>
<xsl:value-of select="$indent1"/><xsl:text> * constructor out of a string array.</xsl:text><xsl:value-of select="$newline"/>
<xsl:value-of select="$indent1"/><xsl:text> * @param sa</xsl:text><xsl:value-of select="$newline"/>
<xsl:value-of select="$indent1"/><xsl:text> *            the string array</xsl:text><xsl:value-of select="$newline"/>
<xsl:value-of select="$indent1"/><xsl:text> */</xsl:text><xsl:value-of select="$newline"/>
...
<xsl:if test="$codegen != 'split'">
	...
	<xsl:if test="@abstract = 'true'">
		<xsl:value-of select="$indent1"/><xsl:text>@SuppressWarnings("unused")</xsl:text><xsl:value-of select="$newline"/>
	</xsl:if>
	<xsl:value-of select="$indent1"/><xsl:text>private static TypeRapidBean type = TypeRapidBean.createInstance(</xsl:text><xsl:value-of select="$classname"/><xsl:text>.class);</xsl:text><xsl:value-of select="$newline"/>
</xsl:if>
...
<xsl:value-of select="$newline"/>
<xsl:value-of select="$indent1"/><xsl:text>/**</xsl:text><xsl:value-of select="$newline"/>
<xsl:value-of select="$indent1"/><xsl:text> * @return value of Property '</xsl:text>
<xsl:value-of select="@name"/>
<xsl:text>'</xsl:text><xsl:value-of select="$newline"/>
<xsl:value-of select="$indent1"/><xsl:text> */</xsl:text><xsl:value-of select="$newline"/>

<xsl:if test="(@type = 'choice' and @multiple = 'true') or @type = 'association' or @type = 'associationend'">
	<xsl:value-of select="$indent1"/><xsl:text>@SuppressWarnings("unchecked")</xsl:text><xsl:value-of select="$newline"/>
</xsl:if>
			...
			<xsl:when test="$impl = 'Simple'">
				...
				<xsl:choose> <!-- switch over types -->
					...
					<xsl:when test="@type = 'integer'">
						<xsl:message terminate="yes">code generation getter for integer property not yet implemented</xsl:message>
					</xsl:when>
					...
					<xsl:when test="@type = 'date'">
						<xsl:value-of select="$indent2"/><xsl:text>if (this.</xsl:text><xsl:value-of select="@name"/><xsl:text> == null) {</xsl:text><xsl:value-of select="$newline"/>
						<xsl:value-of select="$indent3"/><xsl:text>return null;</xsl:text><xsl:value-of select="$newline"/>
						<xsl:value-of select="$indent2"/><xsl:text>} else {</xsl:text><xsl:value-of select="$newline"/>
						<xsl:value-of select="$indent3"/><xsl:text>return new java.util.Date(this.</xsl:text><xsl:value-of select="@name"/><xsl:text>.getTime());</xsl:text><xsl:value-of select="$newline"/>
					...
					<xsl:when test="@type = 'choice'">
						<xsl:message terminate="yes">code generation getter for choice property not yet implemented</xsl:message>
					</xsl:when> <!-- @type = 'choice' -->

					<xsl:when test="@type = 'association' or @type = 'associationend'">
						<xsl:message terminate="yes">code generation getter for association property not yet implemented</xsl:message>
...
<xsl:choose>
	...
	<xsl:otherwise>
		...
		<xsl:value-of select="$newline" />
		<xsl:value-of select="$indent1" /><xsl:text>/**</xsl:text><xsl:value-of select="$newline" />
		<xsl:value-of select="$indent1" /><xsl:text> * setter for Property '</xsl:text><xsl:value-of select="@name" /><xsl:text>'.</xsl:text><xsl:value-of select="$newline" />
		<xsl:value-of select="$indent1" /><xsl:text> * @param argValue</xsl:text><xsl:value-of select="$newline" />
		<xsl:value-of select="$indent1" /><xsl:text> *            value of Property '</xsl:text><xsl:value-of select="@name" /><xsl:text>' to set</xsl:text><xsl:value-of select="$newline" />
		<xsl:value-of select="$indent1" /><xsl:text> */</xsl:text><xsl:value-of select="$newline" />
		<xsl:value-of select="$indent1" /><xsl:text>public void set</xsl:text>
		<xsl:value-of select="translate(substring(@name,1, 1), 'abcdefghijklmnopqrstuvwxyz', 'ABCDEFGHIJKLMNOPQRSTUVWXYZ')" />
		<xsl:value-of select="substring(@name,2, string-length(@name) - 1)" />
		<xsl:text>(</xsl:text>
		<xsl:text>final </xsl:text>
		<xsl:call-template name="javaType">
			<xsl:with-param name="mode">set</xsl:with-param>
		</xsl:call-template>
		<xsl:text> argValue) {</xsl:text><xsl:value-of select="$newline" />
		...
		<xsl:choose>
			...
			<xsl:when test="$impl = 'Simple'"> <!-- implementation = Simple -->
				...
				<xsl:choose> <!-- switch over types -->
					...
					<xsl:when test="@type = 'boolean'">
						<xsl:text>Property.createInstance(getType().getPropertyType("</xsl:text><xsl:value-of select="@name"/><xsl:text>"), this).setValue(argValue);</xsl:text><xsl:value-of select="$newline"/>
					</xsl:when>
...
<xsl:if test="(@type = 'association' or @type = 'associationend') and ((count(@maxmult) = 0) or (@maxmult > 1))">
	<xsl:value-of select="$indent1"/><xsl:text>/**</xsl:text><xsl:value-of select="$newline"/>
	<xsl:value-of select="$indent1"/><xsl:text> * add method for Property '</xsl:text>
	<xsl:value-of select="@name"/>
	<xsl:text>'.</xsl:text><xsl:value-of select="$newline"/>
	<xsl:value-of select="$indent1"/><xsl:text> * @param bean the bean to add</xsl:text><xsl:value-of select="$newline"/>
	<xsl:value-of select="$indent1"/><xsl:text> */</xsl:text>
	<xsl:value-of select="$newline"/>
	<xsl:value-of select="$indent1"/><xsl:text>public void add</xsl:text>
	<xsl:call-template name="singularPropName"/>
	<xsl:text>(final </xsl:text>
	<xsl:value-of select="@targettype"/>
	<xsl:text> bean) {</xsl:text><xsl:value-of select="$newline"/>
	<xsl:value-of select="$indent2"/><xsl:text>((org.rapidbeans.core.basic.PropertyCollection) this.</xsl:text>
	<xsl:value-of select="@name"/>
	<xsl:text>).addLink(bean);</xsl:text><xsl:value-of select="$newline"/>
	<xsl:value-of select="$indent1"/><xsl:text>}</xsl:text><xsl:value-of select="$newline"/>
</xsl:if>
...
<xsl:if test="(@type = 'association' or @type = 'associationend') and ((count(@maxmult) = 0) or (@maxmult > 1))">
	<xsl:value-of select="$indent1"/><xsl:text>/**</xsl:text><xsl:value-of select="$newline"/>
	<xsl:value-of select="$indent1"/><xsl:text> * remove method for Property '</xsl:text>
	<xsl:value-of select="@name"/>
	<xsl:text>'.</xsl:text><xsl:value-of select="$newline"/>
	<xsl:value-of select="$indent1"/><xsl:text> * @param bean the bean to add</xsl:text><xsl:value-of select="$newline"/>
	<xsl:value-of select="$indent1"/><xsl:text> */</xsl:text>
	<xsl:value-of select="$newline"/>
	<xsl:value-of select="$indent1"/><xsl:text>public void remove</xsl:text>
	<xsl:call-template name="singularPropName"/>
	<xsl:text>(final </xsl:text>
	<xsl:value-of select="@targettype"/>
	<xsl:text> bean) {</xsl:text><xsl:value-of select="$newline"/>
	<xsl:value-of select="$indent2"/><xsl:text>((org.rapidbeans.core.basic.PropertyCollection) this.</xsl:text>
	<xsl:value-of select="@name"/>
	<xsl:text>).removeLink(bean);</xsl:text><xsl:value-of select="$newline"/>
	<xsl:value-of select="$indent1"/><xsl:text>}</xsl:text><xsl:value-of select="$newline"/>
</xsl:if>
