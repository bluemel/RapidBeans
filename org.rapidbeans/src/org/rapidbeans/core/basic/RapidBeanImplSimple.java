 ...
 */
public abstract class RapidBeanImplSimple implements RapidBean, Link {
			...
			for (final PropertyCollection colProp : this
				...
				if (minmult > 0) {
					...
					for (int j = 0; j < minmult; j++) {
						colProp.addLink(RapidBeanImplSimple
	...
	private void resetId(final RapidBean newParentBean) {
		...
		for (PropertyCollection colProp : this.getColPropertiesComposition()) {
			...
			for (Object o : (Collection<?>) colProp.getValue()) {
				final RapidBeanImplSimple bean = (RapidBeanImplSimple) o;
				bean.resetId(this);
			}
		}
	}
		...
		try {
			...
			for (PropertyCollection colProp : this.getColProperties()) {
				...
				if (colPropType.isComposition()) {
					...
					Collection<RapidBeanImplSimple> componentBeansToDelete = new ArrayList<RapidBeanImplSimple>();
					for (RapidBeanImplSimple bean : col) {
						componentBeansToDelete.add(bean);
					}
					for (RapidBeanImplSimple bean : componentBeansToDelete) {
						bean.delete();
					}
					componentBeansToDelete.clear();
				} else {
					Collection<RapidBeanImplSimple> beansToUnlink = new ArrayList<RapidBeanImplSimple>();
					for (RapidBeanImplSimple bean : col) {
						beansToUnlink.add(bean);
					}
					for (RapidBeanImplSimple bean : beansToUnlink) {
	...
	public int compareTo(final Link link) {
		...
		if (link instanceof RapidBeanImplSimple) {
			final RapidBeanImplSimple bean = (RapidBeanImplSimple) link;
	...
	public int compareToOld(final Object o) {
		...
		if (o instanceof RapidBeanImplSimple) {
			final RapidBeanImplSimple bean = (RapidBeanImplSimple) o;
	 ...
	 * @return the compare value
	 */
	private int compareIds(final RapidBeanImplSimple bean) {
		int compare;
	 ...
	 */
	private final boolean idtypesComparable(final RapidBeanImplSimple bean) {
		if (this.getType() == bean.getType()) {
			return true;
		}
	 ...
	 *         types
	 */
	private boolean sameSortingProptypes(RapidBeanImplSimple bean,
	...
	public boolean equals(final Object o) {
		...
		try {
			RapidBeanImplSimple bean = (RapidBeanImplSimple) o;
			if (!(this.getType() == bean.getType())) {
				return false;
			}
			...
			for (final String pname : propnames) {
				...
				if (j < propnamessize) {
					if (prop instanceof PropertyCollection) {
						...
						if (col != null) {
							linkedBean = (RapidBeanImplSimple) col.iterator()
									.next();
						} else {
							break;
						}
	...
	public RapidBean clone() {
		RapidBeanImplSimple bClone = RapidBeanImplSimple.createInstance(this
				.getType());
		for (int i = 0; i < this.properties.length; i++) {
				...
				for (final Link link : (Collection<Link>) prop.getValue()) {
					if (link instanceof RapidBeanImplSimple) {
						if (((TypePropertyCollection) prop.getType())
								.isComposition()) {
							((RapidBeanImplSimple) link).trace();
						} else {
							log.info("BEAN LINK: "
									+ ((RapidBeanImplSimple) link)
											.getIdString());
						}
					} else {
