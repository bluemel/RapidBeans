...
public abstract class RapidBeanImplStrict
    ...
    public static RapidBeanImplStrict createInstance(final TypeRapidBean type) {
        ...
        if (clazz == null) {
            ...
            try {
                Constructor<RapidBeanImplStrict> constr = (Constructor<RapidBeanImplStrict>) clazz.getConstructor(CONSTR_PARAMTYPES_STRING_ARRAY);
...
//    }
    ...
    private void resetId(final RapidBean newParentBean) {
        ...
        for (PropertyCollection colProp : this.getColPropertiesComposition()) {
            ...
            for (Object o : (Collection<?>) colProp.getValue()) {
                final RapidBeanImplStrict bean = (RapidBeanImplStrict) o;
                bean.resetId(this);
            }
        }
    }
    ...
    public void delete() {
        ...
        try {
            ...
            if (this.parent != null) {
                ...
                Collection<RapidBeanImplStrict> inverseCol;
                ...
                for (PropertyCollection inverseColProp : this.parent.getColPropertiesComposition()) {
                    ...
                    inverseCol = (Collection<RapidBeanImplStrict>) inverseColProp.getValue();
            ...
            for (PropertyCollection colProp : this.getColProperties()) {
                ...
                if (colPropType.isComposition()) {
                    ...
                    Collection<RapidBeanImplStrict> componentBeansToDelete = new ArrayList<RapidBeanImplStrict>();
                    for (RapidBeanImplStrict bean : col) {
                        componentBeansToDelete.add(bean);
                    }
                    for (RapidBeanImplStrict bean : componentBeansToDelete) {
                        bean.delete();
                    }
                    componentBeansToDelete.clear();
                } else {
                    Collection<RapidBeanImplStrict> beansToUnlink = new ArrayList<RapidBeanImplStrict>();
                    for (RapidBeanImplStrict bean : col) {
                        beansToUnlink.add(bean);
                    }
                    for (RapidBeanImplStrict bean : beansToUnlink) {
    ...
    public int compareTo(final Link link) {
        ...
        if (link instanceof RapidBeanImplStrict) {
            final RapidBeanImplStrict bean = (RapidBeanImplStrict) link;
    ...
    public int compareToOld(final Object o) {
        ...
        if (o instanceof RapidBeanImplStrict) {
            final RapidBeanImplStrict bean = (RapidBeanImplStrict) o;
     ...
     * @return the compare value
     */
    private int compareIds(final RapidBeanImplStrict bean) {
        int compare;
     ...
     */
    private final boolean idtypesComparable(final RapidBeanImplStrict bean) {
        if (this.getType() == bean.getType()) {
            return true;
        }
     ...
     */
    private boolean sameSortingProptypes(RapidBeanImplStrict bean,
    ...
    public boolean equals(final Object o) {
        ...
        try {
            RapidBeanImplStrict bean = (RapidBeanImplStrict) o;
            if (!(this.getType() == bean.getType())) {
                return false;
            }
            ...
            for (final String pname : propnames) {
                ...
                if (j < propnamessize) {
                    if (prop instanceof PropertyCollection) {
                        ...
                        if (col != null) {
                            linkedBean = (RapidBeanImplStrict) col.iterator().next();
                        } else {
                            break;
                        }
                    } else {
                ...
                for (final Link link : (Collection<Link>) prop.getValue()) {
                    if (link instanceof RapidBeanImplStrict) {
                        if (((TypePropertyCollection) prop.getType()).isComposition()) {
                            ((RapidBeanImplStrict) link).trace();
                        } else {
                            log.info("BEAN LINK: "
                                    + ((RapidBeanImplStrict) link).getIdString());
                        }
                    } else {
