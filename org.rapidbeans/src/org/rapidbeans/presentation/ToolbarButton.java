...
public class ToolbarButton extends RapidBeanImplStrict {
	 ...
	 * @return the instance
	 */
	public static ToolbarButton createInstance(final ConfigToolbarButton cfg,
		...
		case swing:
			toolbarButton = new ToolbarButtonSwing(cfg, client, mainWindow,
					resourcePath);
			break;
		case eclipsercp:
	...
	public ToolbarButton(final Application client,
			final ConfigToolbarButton config, final String resourcePath) {
		...
		if (configAction != null && configAction.getParentBean() != null
			...
			((ConfigToolbarButton) configAction.getParentBean())
					.setAction(null);
		}
		if (configAction != null && configAction.getParentBean() != null) {
	...
	public void initProperties() {
		this.name = (org.rapidbeans.core.basic.PropertyString) this
				.getProperty("name");
		this.action = (org.rapidbeans.core.basic.PropertyAssociationend) this
				.getProperty("action");
	...
	private static TypeRapidBean type = TypeRapidBean
			.createInstance(ToolbarButton.class);
		...
		try {
			...
			if (col == null || col.size() == 0) {
				...
				if (link instanceof LinkFrozen) {
					...
					return (org.rapidbeans.service.Action) col.iterator()
							.next();
				}
			}
		} catch (NullPointerException e) {
