    ...
    public ModelComboBoxCollection(final PropertyCollection colProp,
        ...
        for (final RapidBean bean : doc.findBeansByType(targetType.getName())) {
            if (colPropType.getMaxmult() != 1
                ...
                || (((Collection<?>) ((PropertyCollection) bean.getProperty(colPropType.getInverse())).getValue()).size() == 0)
                || (((Collection<?>) ((PropertyCollection) bean.getProperty(colPropType.getInverse())).getValue()).iterator().next() == colProp.getBean())
                ) {
                this.list.add(bean);
            }
        }
