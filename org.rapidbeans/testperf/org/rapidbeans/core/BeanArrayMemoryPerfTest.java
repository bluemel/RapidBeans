...
public final class BeanArrayMemoryPerfTest extends TestCase {
	...
	public void testBeanArrayHeap() throws InterruptedException {
		...
		System.out
				.println("[BeanArrayMemoryPerfTest] setting up 1st bean array...");
		...
		System.out
				.println("[BeanArrayMemoryPerfTest] setting up 2nd bean array");
		System.out
				.println("[BeanArrayMemoryPerfTest]   without releasing the 1st one...");
		// give message a chance to be printed on console before load starts
		Thread.sleep(100);
		RapidBean[] beans2 = new RapidBean[count];
		try {
			for (int i = 0; i < count; i++) {
				beans2[i] = RapidBeanImplStrict
						.createInstance("org.rapidbeans.presentation.MenuItem");
			}
			...
			System.out
					.println("[BeanArrayMemoryPerfTest] got an InvocationTargetException"
							+ " caused by an OutOfMemoryError as expected");
		} catch (OutOfMemoryError e) {
			System.out
					.println("[BeanArrayMemoryPerfTest] got an OutOfMemoryError as expected");
		}

		// release the first array
		System.out
				.println("[BeanArrayMemoryPerfTest] releasing reference to 1st BeanBean array...");
		beans1 = null;

		// set up the second array again
		System.out
				.println("[BeanArrayMemoryPerfTest] trying to set up 2nd bean array again...");
		// give message a chance to be printed on console before load starts
		Thread.sleep(100);
		...
		System.out
				.println("[BeanArrayMemoryPerfTest] finished: garbage collection succeded");
	}
}
